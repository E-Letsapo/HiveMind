...
class IdmApp extends Component {
  ...
  fetchSearchresult = () => {
    ...
    tables.forEach((table) => {
      fetch(`http://localhost:3007/${table}/${searchId}`)
        .then((response) => response.json())
        .then((data) => {
  ...
  fetchTableColumns = (table) => {
    fetch(`http://localhost:3007/${table}/columns`)
      .then((response) => response.json())
      .then((columns) => {
        this.setState((prevState) => ({
          insertFormData: {
  ...
  handleFormSubmit = (event) => {
    ...
    // Check if university_id is 'IDM'
    if (insertFormData.university_id !== 'IDM' || '') {
      console.log('Please enter the correct university_id: IDM');
      return;
    }
...
updateData = () => {
  ...
  fetch(`http://localhost:3007/${selectedTable}/${id}`, {
    method: 'PUT',
  ...
  render() {
    ...
    const filteredData =
      data && data.filter((row) => row.university_id === 'IDM'); // Add null check

...
//APP COMPONENT
class LipamApp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedTable: '',
  ...
  fetchTables = () => {
    ...
    fetch('http://localhost:3004/tables')
  ...
  fetchSearchresult = () => {
    ...
    tables.forEach((table) => {
      fetch(`http://localhost:3004/${table}/${searchId}`)
        .then((response) => response.json())
        .then((data) => {
  ...
  fetchTableColumns = (table) => {
    fetch(`http://localhost:3004/${table}/columns`)
      .then((response) => response.json())
      .then((columns) => {
        this.setState((prevState) => ({
          insertFormData: {
  ...
  handleFormSubmit = (event) => {
    ...
    // Check if university_id is 'LIPAM'
    if (insertFormData.university_id !== 'LIPAM' || '') {
      console.log('Please enter the correct university_id: LIPAM');
      return;
    }
  
    // Send the form data to the server for insertion
    fetch(`http://localhost:3004/${selectedTable}`, {
      method: 'POST',
...
updateData = () => {
  ...
  fetch(`http://localhost:3004/${selectedTable}/${id}`, {
    method: 'PUT',
  ...
  render() {
    ...
    const filteredData =
      data && data.filter((row) => row.university_id === 'LIPAM'); // Add null check
    ...
    return (
      <div className="App">
        <div className="LipamApp-header">
          <img src={logo} className="App-logo" alt="logo" />
          <h1>LIPAM Database Management System</h1>

...
//APP COMPONENT
class LacApp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedTable: '',
  ...
  fetchTables = () => {
    ...
    fetch('http://localhost:3008/tables')
  ...
  fetchData = (table) => {
    ...
    fetch(`http://localhost:3008/${table}`)
      .then((response) => response.json())
      .then((data) => {
  ...
  fetchSearchresult = () => {
    ...
    tables.forEach((table) => {
      fetch(`http://localhost:3008/${table}/${searchId}`)
        .then((response) => response.json())
        .then((data) => {
  ...
  fetchTableColumns = (table) => {
    fetch(`http://localhost:3008/${table}/columns`)
      .then((response) => response.json())
      .then((columns) => {
        this.setState((prevState) => ({
          insertFormData: {
  ...
  handleFormSubmit = (event) => {
    ...
    // Check if university_id is 'LAC'
    if (insertFormData.university_id !== 'LAC' || '') {
      console.log('Please enter the correct university_id: LAC');
      return;
    }
  
    // Send the form data to the server for insertion
    fetch(`http://localhost:3008/${selectedTable}`, {
      method: 'POST',
...
updateData = () => {
  ...
  fetch(`http://localhost:3008/${selectedTable}/${id}`, {
    method: 'PUT',
  ...
  render() {
    ...
    const filteredData =
      data && data.filter((row) => row.university_id === 'LAC'); // Add null check
    ...
    return (
      <div className="App">
        <div className="LacApp-header">
          <img src={logo} className="App-logo" alt="logo" />
          <h1>LAC Database Management System</h1>

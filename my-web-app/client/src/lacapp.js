...
class LacApp extends Component {
  ...
  insertData = () => {
    ...
    // Only allow insert operation if the university name is LAC
    if (newData.university_name === 'LAC') {
      ...
      window.alert('University name must be "LAC" for insert operation');
    }
  };
  ...
  render() {
    ...
    const filteredData = data.filter((row) => row.university_name === 'LAC');

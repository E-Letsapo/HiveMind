...
class CasApp extends Component {
  ...
  insertData = () => {
    ...
    // Only allow insert operation if the university name is CAS
    if (newData.university_name === 'CAS') {
      ...
      window.alert('University name must be "CAS" for insert operation');
    }
  };
  ...
  render() {
    ...
    const filteredData = data.filter((row) => row.university_name === 'CAS');

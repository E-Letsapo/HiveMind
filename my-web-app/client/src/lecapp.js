...
//APP COMPONENT
class LecApp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedTable: '',
  ...
  fetchTables = () => {
    ...
    fetch('http://localhost:3006/tables')
  ...
  fetchSearchresult = () => {
    ...
    tables.forEach((table) => {
      fetch(`http://localhost:3006/${table}/${searchId}`)
        .then((response) => response.json())
        .then((data) => {
  ...
  fetchTableColumns = (table) => {
    fetch(`http://localhost:3006/${table}/columns`)
      .then((response) => response.json())
      .then((columns) => {
        this.setState((prevState) => ({
          insertFormData: {
  ...
  handleFormSubmit = (event) => {
    ...
    // Check if university_id is 'LEC'
    if (insertFormData.university_id !== 'LEC' || '') {
      console.log('Please enter the correct university_id: LEC');
      return;
    }
  
    // Send the form data to the server for insertion
    fetch(`http://localhost:3006/${selectedTable}`, {
      method: 'POST',
...
updateData = () => {
  ...
  fetch(`http://localhost:3006/${selectedTable}/${id}`, {
    method: 'PUT',
  ...
  render() {
    ...
    const filteredData =
      data && data.filter((row) => row.university_id === 'LEC'); // Add null check
    ...
    return (
      <div className="App">
        <div className="LecApp-header">
          <img src={logo} className="App-logo" alt="logo" />
          <h1>LEC Database Management System</h1>

...
class LpApp extends Component {
  ...
  insertData = () => {
    ...
    // Only allow insert operation if the university name is Lp
    if (newData.university_name === 'LP') {
      ...
      window.alert('University name must be "LP" for insert operation');
    }
  };
  ...
  render() {
    ...
    const filteredData = data.filter((row) => row.university_name === 'Lp');

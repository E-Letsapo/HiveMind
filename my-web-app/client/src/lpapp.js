...
class LpApp extends Component {
  ...
  fetchTables = () => {
    ...
    fetch('http://localhost:3005/tables')
  ...
  fetchData = (table) => {
    ...
    fetch(`http://localhost:3005/${table}`)
      .then((response) => response.json())
      .then((data) => {
  ...
  fetchSearchresult = () => {
    ...
    tables.forEach((table) => {
      fetch(`http://localhost:3005/${table}/${searchId}`)
        .then((response) => response.json())
        .then((data) => {
  ...
  fetchTableColumns = (table) => {
    fetch(`http://localhost:3005/${table}/columns`)
      .then((response) => response.json())
      .then((columns) => {
        this.setState((prevState) => ({
          insertFormData: {
  ...
  handleFormSubmit = (event) => {
    ...
    // Check if university_id is 'LP'
    if (insertFormData.university_id !== 'LP' || '') {
      console.log('Please enter the correct university_id: LP');
      return;
    }
  
    // Send the form data to the server for insertion
    fetch(`http://localhost:3005/${selectedTable}`, {
      method: 'POST',
...
updateData = () => {
  ...
  fetch(`http://localhost:3005/${selectedTable}/${id}`, {
    method: 'PUT',
  ...
  render() {
    ...
    const filteredData =
      data && data.filter((row) => row.university_id === 'LP'); // Add null check
    ...
    return (
      <div className="App">
        <div className="LpApp-header">
          <img src={logo} className="App-logo" alt="logo" />
          <h1>LP Database Management System</h1>
